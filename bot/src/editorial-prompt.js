...
export function buildUserPrompt({ topic, researchData, contentType, template, today }) {
  ...
  return [
    ...
    `Cubra cada um dos ${template.structure.length} eixos obrigatórios, mas não crie parágrafos ou seções apenas para atingir extensão.`,
    ...
    "A saída usa o contrato editorial v2: cada seção precisa ter target_question, claims com statement/source_ids/evidence_quote/confidence e internal_links somente quando houver destino contextual real.",
    ...
    '  "sources": [',
    '    { "id": "src-1", "name": "Fonte", "type": "manufacturer", "url": "https://...", "accessed_at": "YYYY-MM-DD" }',
    "  ],",
    '  "frontmatter": {',
    ...
    "  },",
    '  "sections": [',
    '    { "heading": "Intertítulo específico e atraente, nunca um rótulo genérico", "target_question": "Qual decisão esta seção ajuda o leitor a tomar?", "content": "...", "claims": [{ "statement": "Afirmação factual", "source_ids": ["src-1"], "evidence_quote": "Trecho literal de pelo menos doze caracteres", "confidence": "high" }], "internal_links": [{ "url": "/guias/exemplo/", "anchor": "guia relacionado", "reason": "Aprofunda a decisão desta seção" }] }',
    "  ],",
    '  "imagePlan": [',
